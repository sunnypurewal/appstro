...
struct Screenshots: AsyncParsableCommand {
	...
	@Option(name: .long, help: "The device type to process (iphone or ipad).")
	var device: String?
	
	@Flag(name: .long, help: "Process local screenshots, adding titles and bezels without uploading.")
	var process: Bool = false

	@Flag(name: .long, help: "Upload processed screenshots without local processing.")
	var upload: Bool = false

	func run() async throws {
		if process && upload {
			UI.error("The --process and --upload flags cannot be used together.")
			return
		}
		if upload && copy != nil {
			UI.error("The --copy flag cannot be used with --upload. Process screenshots first.")
			return
		}
		...
		let isProcessingOnly = process && !upload
		let isUploadingOnly = upload && !process
		let isProcessingAndUploading = !process && !upload
		...
		if isProcessingOnly {
			guard let appVersion = appVersion else {
				UI.error("The --app-version option is required when using --process.")
				return
			}
		...
		let releaseDir = try await Environment.live.project.ensureReleaseDirectory(at: root, version: versionString)
		...
		if isProcessingOnly || isProcessingAndUploading {
			...
			if let copyPath = copy {
					...
					for file in files {
						...
						if let imageSource = CGImageSourceCreateWithURL(file as CFURL, nil),
						   ...
						   let height = imageProperties[kCGImagePropertyPixelHeight] as? CGFloat {
							...
							let targetDeviceType = (aspectRatio < 1.6) ? "ipad" : "iphone"
							
							// If --device is specified, only copy if it matches
							if let device = device, device.lowercased() != targetDeviceType {
								continue
							}

							let destinationDir = (targetDeviceType == "ipad") ? ipadDir : iphoneDir
				...
				if !newFiles.isEmpty {
					...
					for file in newFiles {
						...
						if let existingTitle = currentTitles[keyword], !existingTitle.isEmpty {
							continue
						}
			...
			if let device = device {
				let files = device.lowercased() == "ipad" ? ipadFiles : iphoneFiles
				if files.isEmpty {
					UI.error("No \(device.lowercased()) screenshots found. Please pass in a directory containing screenshots using --copy <path_to_screenshots>")
					return
				}
			} else if iphoneFiles.isEmpty && ipadFiles.isEmpty {
			...
			try await UI.step("Processing screenshots", emoji: "🎨") {
				let deviceTypesToProcess = (device != nil) ? [device!.lowercased()] : ["iphone", "ipad"]
				
				for deviceType in deviceTypesToProcess {
					...
					for file in imageFiles {
						...
						guard let bezelInfo = Environment.live.bezel.bezelInfo(for: deviceType, isLandscape: isLandscape) else { continue }
		...
		if isUploadingOnly || isProcessingAndUploading {
			guard let service = service, let versionId = versionId else {
				UI.error("An internal error occurred trying to prepare for upload.")
				return
			}

			if !FileManager.default.fileExists(atPath: processedDir.path) {
				UI.error("Processed screenshots directory not found at '\(processedDir.path)'.\nPlease run with --process first or without any flags.")
				return
			}

			// 7. Upload to App Store Connect
			try await UI.step("Uploading to version \(versionString)", emoji: "🚀") {
				let deviceTypes = (device != nil) ? [device!.lowercased()] : nil
				try await service.screenshots.uploadScreenshots(versionId: versionId, processedDirectory: processedDir, deviceTypes: deviceTypes)
			}
		}
		
		if isProcessingOnly {
			UI.success("Screenshots processed successfully! Find them at: \(processedDir.path)")
		} else if isUploadingOnly {
			UI.success("Screenshots uploaded successfully!")
		} else {
			UI.success("Screenshots processed and uploaded successfully!")
		}
	}
