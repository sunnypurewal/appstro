				...
				if let draft = try await service.versions.findDraftVersion(for: app.id) {
					...
					if eligibleStates.contains(draft.state) {
						if let root = Environment.live.project.findProjectRoot() {
							_ = try? await Environment.live.project.ensureReleaseDirectory(at: root, version: draft.version)
						}
...
extension Metadata {
	struct All: AsyncParsableCommand {
		...
		func run() async throws {
			let service = try Environment.live.asc(Environment.live.bezel)
	...
	struct Generate: AsyncParsableCommand {
		...
		func run() async throws {
			...
			let answer = UI.prompt("Upload?").lowercased()
			if answer == "y" {
				var contactInfo = try await service.reviews.fetchContactInfo()
				
				if contactInfo.firstName == nil {
					contactInfo.firstName = UI.prompt("👤 Enter App Review Contact First Name")
				}
				if contactInfo.lastName == nil {
					contactInfo.lastName = UI.prompt("👤 Enter App Review Contact Last Name")
				}
				if contactInfo.email == nil {
					contactInfo.email = UI.prompt("📧 Enter App Review Contact Email")
				}
				if contactInfo.phone == nil {
					contactInfo.phone = UI.prompt("📱 Enter App Review Contact Phone (e.g., +1 555-555-5555)")
				}
