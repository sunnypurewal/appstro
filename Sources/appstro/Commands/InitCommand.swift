...
struct Init: AsyncParsableCommand {
	...
	func run() async throws {
		...
		let projectName = UI.prompt("app name", defaultValue: defaultName)
		let description = UI.prompt("description")
		let keywordsString = UI.prompt("keywords")
		...
		let ok = UI.prompt("Is this OK?", defaultValue: "yes")
		...
		try await UI.step("Initializing project", emoji: "🚀") {
			...
			if existingProjectName == nil {
				...
				try (sourcesPath + "App.swift").write(appSwift.data(using: .utf8)!)
				try (sourcesPath + "ContentView.swift").write(contentViewSwift.data(using: .utf8)!)
				...
				if xcodeProjectPath.exists {
					let overwrite = UI.prompt("\(xcodeProjectPath.lastComponent) already exists. Overwrite?", defaultValue: "no")
					if overwrite.lowercased() == "yes" || overwrite.lowercased() == "y" {
